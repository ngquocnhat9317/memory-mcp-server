...
- `GUIDELINES.md` bumped to `2026-07-11.v4`: feedback reporting described as always-on for successful feedback; report tools clarified to need `MEMORY_TELEMETRY=on` for full funnels; task-start guidance now teaches the relevance-ranked recall and the `source` provenance field (replay origins with `reasoning_get_trace`). The README AGENTS snippet gained the same provenance hint.
